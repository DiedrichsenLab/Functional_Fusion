    ...
    for _, id in T.iterrows():
        ...
        for session in np.arange(4):
            ...
            reginfo = pd.read_csv(dest_base_dir /
                        f'derivatives/{id.participant_id}/estimates/ses-{session+1:02d}/{id.participant_id}_ses-{session+1:02d}_reginfo.tsv', sep='\t')
            resms = []
            for _, run, _, pe, _, reg_id in reginfo.iterrows():
            # for run in np.arange(info.run.max()):
                ...
                #     dest = dest_base_dir / \
                #         f'derivatives/{id.participant_id}/estimates/ses-{session+1:02d}/{id.participant_id}_ses-{session+1:02d}_run-{run+1:02d}_reg-{reg:02d}_beta.nii.gz'
