...
from Functional_Fusion.dataset import DataSetLanguageLocalizer, DataSetPontine
...
data_dir = base_dir + '/Language_localizer'
atlas_dir = base_dir + '/Atlases'


# def show_langloc_group(type='TaskHalf', atlas='SUIT3', cond=0, info_column='task_name', savefig=False):
#     mask = atlas_dir + '/tpl-SUIT/tpl-SUIT_res-3_gmcmask.nii'
#     suit_atlas = am.AtlasVolumetric('cerebellum', mask_img=mask)
#     LL_dataset = DataSetLanguageLocalizer(data_dir)
#     C = nb.load(LL_dataset.data_dir.split('/{0}')[0] +
#                 f'/group/group_space-{atlas}_{type}.dscalar.nii')
#     D = pd.read_csv(LL_dataset.data_dir.split('/{0}')[0] +
...
def extract_langloc_suit(atlas='SUIT3'):
    LL_dataset = DataSetPontine(data_dir)
    LL_dataset.extract_all_suit(type,atlas)
