...
def get_hcp_data(res=162, index=range(0, 100), runs=[0,1,2,3], refix=False):
    ...
    for s in T.participant_id[index]:
        ...
        coef = hcp_dataset.get_cereb_connectivity(s, atlas_map, surf_parcel,
                                                  runs=runs, refix=refix)
        # Average or Contatenate across runs? Not sure.
        # For computation efficiency, use average
...
def learn_runs(K=10, max_iter=100, runs=np.arange(1, 17), sub=None, do_plot=True):
    ...
    Args:
        ...
        runs: the number of runs data to use (accumulative)
        sub: None
        do_plot: plot if True
    Returns:
        group_baseline: The cosine error for the group baseline
        lower_bound: The noise floor
        cos_em: The cosine error for the data only
        cos_complete: The cosine error for data+prior
...
def learn_hcp_mdtb(K=10, max_iter=100, runs=np.arange(1, 17), sub=None, do_plot=True):
    ...
    Data_hcp = get_hcp_data(res=162, index=range(0, 80), runs=[0,1,2,3])
    # Make arrangement model and initialize the prior from the MDTB map
    P = Data.shape[2]
    prior_w = 7.0  # Weight of prior
    mdtb_parcel, mdtb_colors = get_mdtb_parcel(do_plot=False)
    ...
    M.pre_train([Data_1, Data_hcp], iter=10, prior=mdtb_prior)
    M, ll, theta, U_hat = M.fit_em(Y=[Data_1, Data_hcp], iter=max_iter, tol=0.00001,
    ...
    for i in runs:
        indices.append(np.asarray(np.where(D_cv.run==i)).reshape(-1))
        acc_run_idx = np.concatenate(indices).ravel()
        M.emissions[0].X = Xdesign_cv[acc_run_idx,:]

        # Infer on current accumulated runs data
        U_hat_em = M.emissions[0].Estep(Y=Data_cv[:,acc_run_idx,:], pure_compute=True)
        U_hat_complete, _ = M.arrange.Estep(U_hat_em)
        # U_hat_complete, _ = M.Estep(Y=Data_cv[:,acc_run_idx,:])
        uhat_em_all.append(U_hat_em)
        uhat_complete_all.append(U_hat_complete)
...
def learn_mdtb_hcp(K=10, max_iter=100, sub_train=None, sub_test=None, do_plot=True):
    """Run this function to replicate the figure 5 in the nips paper
    Args:
        K: the number of parcels
        max_iter: maximum iterations for EM learning
    ...
    ############################# Cross-validation starts here #############################
    # Make inference on the selected run's data - run_infer
    # cos_em, cos_complete, uhat_em_all, uhat_complete_all = [],[],[],[]
    #
    # # Infer on current accumulated runs data
    # U_hat_em = M.emissions[1].Estep(Y=Data_hcp, pure_compute=True)
    # U_hat_complete, _ = M.arrange.Estep(U_hat_em)
    # # U_hat_complete, _ = M.Estep(Y=Data_cv[:,acc_run_idx,:])
    # uhat_em_all.append(U_hat_em)
    # uhat_complete_all.append(U_hat_complete)
    ...
    # UEM = pt.argmax(U_hat_em, dim=1)+1
    # UALL = pt.argmax(U_hat_complete, dim=1) + 1
    # _plot_maps(UEM, sub=0, color=True, render_type='matplotlib',
    #            save=f'emiloglik_only_hcp.pdf')
    # _plot_maps(UALL, sub=0, color=True, render_type='matplotlib',
    #            save=f'all_hcp.pdf')
    #
    # # Calculate cosine error/u abs error between another test data
    # # and U_hat inferred from testing
    # coserr_Uem = ev.coserr(pt.tensor(data_sc2), em_model2.V, pt.softmax(U_hat_em, dim=1),
    #                        adjusted=True, soft_assign=True)
    # coserr_Uall = ev.coserr(pt.tensor(data_sc2), em_model2.V, U_hat_complete,
    #                         adjusted=True, soft_assign=True)
    #
    # cos_em.append(coserr_Uem)
    # cos_complete.append(coserr_Uall)

    return group_baseline, lower_bound

def figure_indiv_group(): 
    D = pd.read_csv('indiv_group_err.csv')
...
if __name__ == "__main__":
    ...
    #     b = np.arange(i*20, (i+1)*20)
    #     A = [j for j in a if j not in b]
    #     gbase, lb = learn_mdtb_hcp(K=10, sub_train=A, sub_test=b)
    #
